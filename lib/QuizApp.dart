  ...
  Future<void> _loadQuestions() async {
    try {
      ...
      if (e.message.contains('Unable to load asset')) {
        print("Error ,path or name file not correct");
      }
    }
  }
    ...
    while (i < lines.length) {
      ...
      while (i < lines.length &&
          (lines[i].startsWith('a.') ||
              ...
              lines[i].startsWith('c.') ||
              lines[i].startsWith('d.'))) {
        choice.add(lines[i].substring(3).trim());
        i++;
      }
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.stretch,
              children:
                  (_questions[_currentQuestionIndex]['choice'] as List<String>)
                      .map((option) {
                return Container(
                  height: 53,
